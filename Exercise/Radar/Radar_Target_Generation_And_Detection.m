...
Mix_pure = zeros(1,length(t)); %beat signal
...
for i=1:length(t)  
    ...
    update_R = zeros(1, num_of_targets);
    
    for trgIdx = 1:num_of_targets
        update_R(trgIdx) = R(trgIdx) + v(trgIdx)*current_time;
    end
    ...
    for trgIdx = 1:num_of_targets
        trip_time(trgIdx) = 2*update_R(trgIdx)/c;
    end
    ...
    Mix_pure(i) = 0;
    for trgIdx = 1:num_of_targets
        Mix_pure(i) = Mix_pure(i) + cos(2*pi*(((2*slope*update_R(trgIdx)*current_time)/c) + ((2*fc*v(trgIdx)*current_time)/c)));
    end
